    ...
    def mass(self, event):
        ...
        if (event.totalWeight > 2):
            ...
            if self._measureCnt == WEIGHT_SAMPLES:
                ...
                self._measureCnt = 0
                self._weight = self._weight + 10.5
                # il parametro serve a tarare la bilancia
                self._weight = self._weight * 0.45
                print(str(round(self._weight, 1)) + " kg")
...
class TelegramBotClass:
    ...
    def handle(self, msg):
        ...
        tmp = "il tuo peso è: " + str(round(self.proc._weight, 1)) + " kg"
        self.ScaleBot.sendMessage(chat_id, tmp)
